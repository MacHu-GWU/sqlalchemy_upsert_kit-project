...
class InsertOrReplaceHelper(UpsertHelper):
    def execute_core_upsert_logic(
        ...
        inner = sa.select(self.table.c[self.pk_name]).join(
            self.temp_table,
            self.table.c[self.pk_name] == self.temp_table.c[self.pk_name],
        )
        stmt = self.table.delete().where(self.table.c[self.pk_name].in_(inner))
        res = conn.execute(stmt)
        try:
            self.replaced_rows = res.rowcount if res.rowcount is not None else 0
        ...
        stmt = self.table.insert().from_select(
            ...
            sa.select(*list(self.temp_table.columns.values())),
        )
        res = conn.execute(stmt)
        try:
            total_inserted = res.rowcount if res.rowcount is not None else 0
            self.inserted_rows = total_inserted - self.replaced_rows
        except:  # pragma: no cover
            self.inserted_rows = len(self.values) - self.replaced_rows
