...
class InsertOrIgnoreHelper(UpsertHelper):
    def execute_core_upsert_logic(
        ...
        stmt = self.table.insert().from_select(
            list(self.temp_table.columns.keys()),
            sa.select(self.temp_table)
            .select_from(
                self.temp_table.outerjoin(  # LEFT JOIN to find non-matches
                    self.table,
                    self.temp_table.c[self.pk_name] == self.table.c[self.pk_name],
                )
            )
            .where(
        ...
        try:
            self.inserted_rows = res.rowcount if res.rowcount is not None else 0
            self.ignored_rows = len(self.values) - self.inserted_rows
        except:  # pragma: no cover
            self.inserted_rows = 0
            self.ignored_rows = len(self.values)
